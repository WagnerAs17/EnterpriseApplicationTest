...
{
    public class UpdateCaminhaoDto : BaseDto
    {
        ...
        {
            AdicionarErroResult(new UpdateCaminhaoValidation().Validate(this));

            return ObterValidationResult().IsValid;
        }
    }

    public class UpdateCaminhaoValidation : AbstractValidator<UpdateCaminhaoDto>
    {
        public UpdateCaminhaoValidation()
        {
            RuleFor(x => x.Id)
                .NotEmpty()
                .WithMessage(CaminhaoValidationMessage.IdObrigatorio);
